...
flowchart TD
    ...
    subgraph Query["Question answering"]
        ...
        L --> M
        M --> Q{"Usable corpus answer found?"}
        Q -->|"Yes"| N["Grounded prompt with range, scope relation,<br/>edition, provenance, and rights"]
        Q -->|"No"| R["Separate internal-knowledge prompt<br/>with ungrounded-answer warning"]
        N --> O["Local LLM grounded answer"]
        O --> P["Validated citations and deterministic<br/>web evidence notices"]
        R --> S["Local LLM internal answer<br/>without corpus citations"]
    end
...
If retrieval returns no evidence, or grounded generation signals that loose
matches do not answer the core question, the application calls the local model
once with a separate internal-knowledge prompt. That answer is explicitly
labeled as ungrounded, contains no corpus citations or evidence notices, and
warns that score-, edition-, and measure-specific details may be inaccurate.
The corpus-grounded prompt is never weakened or mixed with this fallback. If
the local model is unavailable, the application returns a model-unavailable
message instead.
...
notices remain aligned. If even one complete record does not fit, the last
valid record is retained and returned as a cited extractive answer.
...
- `generation_mode`: whether the response was generated by the local LLM or
  returned as retrieval-only output;
- `answer_basis`: `retrieved_evidence`, `internal_knowledge`, or the applicable
  retrieval/unavailable fallback, so consumers never confuse an internal
  answer with a grounded one;
...
| Reusable application service | `soprano_qa/service.py`: `ask()`, `corpus_stats()`, `model_status()` |
